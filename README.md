...
The TypeScript CLI currently provides help, version output, suggestions for misspelled commands, and an interactive setup. License access is checked with the Themistic server; verified keys are saved in the system credential store. Model packages can be downloaded and verified. The local API is not available yet, and there is no published release.
