...
test('license commands are discoverable and never accept a key as an argument', function () {
    for (var args of [['license'], ['license', 'set', '--help'], ['license', 'status', '--help']]) {
        var result = spawnSync(CLI_PATH, args, { encoding: 'utf8' });
        assert.equal(result.status, 0);
        assert.match(result.stdout, /license/);
    }
    var piped = spawnSync(CLI_PATH, ['license', 'set'], { encoding: 'utf8' });
    assert.equal(piped.status, 0);
    assert.match(piped.stdout, /interactive terminal/);
    for (var args of [['license', 'set', 'TEST-ONLY-KEY'], ['license', 'TEST-ONLY-KEY'], ['license', 'set', '--TEST-ONLY-KEY']]) {
        var argument = spawnSync(CLI_PATH, args, { encoding: 'utf8' });
